...
"$BT/aapt2" link \
  ...
  --version-code 6 --version-name 0.5.1 \
...
"$BT/apksigner" sign --ks "$KS" --ks-pass pass:android --key-pass pass:android \
  --out "$OUT/医疗价格查询-0.5.1.apk" "$OUT/aligned.apk"

"$BT/apksigner" verify --print-certs "$OUT/医疗价格查询-0.5.1.apk" | head -3

echo "== DONE =="
ls -la "$OUT/医疗价格查询-0.5.1.apk"

...
离线、移动端优先的医疗服务价格表查询系统。把《成都市医疗服务项目价格汇编（2024版）》全量 8124 项 PDF 价格表解析成结构化数据，提供关键词/编码/拼音/首字母/别名/智能子序列/模糊检索、多版本横向对比、以及点结果由内置 PDF 经 pdf.js 现场渲染原文页并高亮关键数据核验。全程本地、数据不出端。

## 架构（v2 · 混合）

```
桌面数据管线(Python/PyMuPDF)  ──►  标准数据包(data/proto_data.json) + doc.pdf  ──►  安卓 APK(WebView)
   解析 1105 页 / 8124 项              结构化项目 + 五级价 + 高亮框(PDF点坐标)          内置数据 · 离线检索 · pdf.js 按需渲染原文页并高亮
```
...
| :-- | :-- |
| `docs/` | v2 设计文档 |
| `data/proto_data.json` | 预建标准数据包（8124 项全量：双编码 / 名称 / 内涵除外 / 五级价原文 / 高亮框 PDF 点坐标） |
| `data/doc.pdf` | 原始 PDF（内置随 App 分发，pdf.js 离线按需渲染原文页） |
| `vendor/pdfjs/` | pdf.js 3.11（UMD，兼容旧 WebView），构建时拷入 prototype/ 与 assets/ |
...
- **检索（P1）**：默认匹配四列（国家项目编码 / 国家项目名称 / 项目编码 / 项目名称）；其余列（内涵 / 除外 / 单位 / 说明 / 价格 / 备注）可勾选纳入匹配。支持关键词 / 拼音全拼 / 首字母 / 别名 / **智能子序列（缩写·同音·汉字+拼音混合，如「单远珍」「单x」命中「单学科远程会诊」）** / 编辑距离模糊；命中类型标注。
- **多版本（P2）**：结果按版本横向分列五级价（三甲/三乙/二甲/二乙/二乙以下）；价格保留原文（含 `30%`、`不超过420` 等非数字）。
- **原文核验（P3）**：内置原始 PDF，pdf.js 现场矢量渲染对应页，自动高亮名称行与五级价，支持缩放/拖动/双指捏合/双击/定位（矢量放大清晰）。
- **别名自管**：搜索选项目 → 增/改/删别名，localStorage 持久化，检索即时生效。
- **离线（P4）**：无 INTERNET 权限，零业务网络请求。

## 说明

- 原始 PDF 为公开政府数据，已作为 `data/doc.pdf` 内置（App 离线渲染原文页所需）。
- 当前内置 2024 版全量 8124 项真实数据；多版本对比在导入其它年份价格表后展开。
